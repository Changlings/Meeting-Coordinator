$(document).ready(function () {
  ...
  $('.delete-meeting').click(function (e) {
    if (confirm("Are you sure you wish to remove this meeting? This cannot be undone!")) {
      // TODO: SERVER LOGIC (REMOVE MEETING USE CASE)
  ...
  $(document).on({
    click: function () {
      ...
      $.getJSON("/Home/Meeting", { id: id }, function (response) {
        ...
        $('#detail-time').text(moment(parseInt(/-?\d+/.exec(response.startTime))).format("MMMM Do YYYY, h:mm:ss a") +
            " to " + moment(parseInt(/-?\d+/.exec(response.endTime))).format("MMMM Do YYYY, h:mm:ss a"));
        // Show the modal
